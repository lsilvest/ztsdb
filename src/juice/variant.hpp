...
namespace Juice
{
  ...
  {
    bad_get(int i) : s(std::string("bad get for type " + std::to_string(i))) { }
  ...
  get (Variant<First, Types...>& var)
  {
    ...
    if (t == nullptr){throw bad_get(var.which());}
  ...
  >
  typename std::remove_reference<Visitor>::type::result_type
  apply_visitor_binary(Visitor&& visitor, Visitable1&& v1, Visitable2&& v2, Args&&... args)
  {
    ...
    return apply_visitor(v, std::forward<Visitable2>(v2), std::forward<Args>(args)...);
  }
