 ...
 *
 * Modifications:
 * - added automated test registration
 * - changed test macros to be similar to crpcut, so the two test frameworks 
...
/* The following is no longer true; tests are declared with the macros TEST
 * and get automatically registered:
 * /
 ...
 *   }
 */
/* TEST_CHECK changed to ASSERT_TRUE so as to mimic crpunit */
#define ASSERT_TRUE_(cond,...)  test_check__((cond), __FILE__, __LINE__, __VA_ARGS__)
#define ASSERT_TRUE(cond)       test_check__((cond), __FILE__, __LINE__, "%s", #cond)


/* Macros for defining test cases that regiter automatically: */
#define CUTEST_CONCAT_DETAIL(x, y) x##y
#define CUTEST_CONCAT(x, y) CUTEST_CONCAT_DETAIL(x, y)
#define CUTEST_MAKE_UNIQUE(x) CUTEST_CONCAT(x, __LINE__)
#define CUTEST_TEST CUTEST_CONCAT(test, __LINE__)

#define TEST(a)                                         \
  void CUTEST_TEST();                                   \
  Registrar CUTEST_MAKE_UNIQUE(reg) (#a, CUTEST_TEST);	\
  void CUTEST_TEST()
...
// info to register a test:
struct test__ {
  const char* name;
  std::function<void()> func;
};

// allows to register tests (name, func) in a static vector:
struct Registrar {
  Registrar(const char* name, std::function<void()> f_p) {
    test_list__.push_back(new test__{name, f_p});
  }

  static std::vector<test__*> test_list__;
};
std::vector<test__*> Registrar::test_list__;
...
{
    printf("Unit tests:\n");
    for(auto test : Registrar::test_list__)
        printf("  %s\n", test->name);
}
...
test_do_run__(const struct test__* test)
{
    ...
    if(test_verbose_level__ >= 3) {
        ...
        n = test_print_in_color(CUTEST_COLOR_DEFAULT_INTENSIVE__, "Test %-50s ", test->name);
...
main(int argc, char** argv)
{
    ...
    test_count__ = Registrar::test_list__.size();
